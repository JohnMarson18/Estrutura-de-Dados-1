...
| :--- | :--- | :--- |
| `/agenda_alocacao_dinamica` | Agenda de contatos com redimensionamento automático. | Uso de `realloc` para aumentar a capacidade do vetor dinamicamente. |
| `/agenda_tad_modularizada` | Versão da agenda separada em interface (`.h`) e implementação (`.c`). | Aplicação do conceito de **Tipo Abstrato de Dados (TAD)**. |
| `/tda_conjuntos_dinamicos` | Implementação da estrutura matemática de Conjuntos. | Operações de União, Interseção e Diferença usando vetores dinâmicos. |
...
| `/leitura_arquivo_lista_encadeada` | Importação de dados de alunos. | Leitura de arquivo `.txt` e conversão para nós de lista. |
...
| :--- | :--- | :--- |
| `/ordenacao_basica_simples` | Implementação pura dos algoritmos. | Vetores pequenos para validação lógica. |
| `/ordenacao_vetor_grande` | Teste de carga com 50.000 elementos. | Comportamento com grandes volumes de dados (aleatórios). |
| `/ordenacao_analise_tempo` | Medição de tempo de execução. | Uso da biblioteca `time.h` e `clock()` para medir milissegundos. |
| `/ordenacao_contagem_custo` | Auditoria de eficiência. | Contagem exata de número de comparações e trocas realizadas. |
